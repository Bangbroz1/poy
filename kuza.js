/* Base Ori : rahman bacot
Script Ori : rahman bacot
Nama Base : MarmutBase
Nama Script : ScStore
Recode : rahman bacot
Youtube Author : rahman bacot
Youtube Recode : rahman bacot
Nama Script Recode : ScStore
Versi Script Ori : 2
Versi Script Recode : 2
Created By Rahmanbotz
*/
...
cd /sdcard
cd BaseV2
git clone https://github.com/Rahmanbotz/node_modules
node .
...
https://github.com/Rahmanbotz/MarmutBase
...
const { getTextSetProses } = require('./lib/setproses');
const { getTextSetDone } = require('./lib/setdone');
...
let botname = '*_｢ MarmutBase 0.1 ｣ 冬_*' //NAMA BOT
let lolkey = 'Atakbotz'
...
let ownername = '*_｢ RahmanGanz ✘ ｣_*' //NAMA OWNER
let owner = '6281287787370' // NOMOR OWNER
let symbol = '*🦅*'
let faketeks = `*_｢ MarmutBase 0.1 ｣ 冬_*` 
...
const ytRahmanbotz = "6281287787370@s.whatsapp.net"
const txt = mek.message.conversation
const botNumber = kuza.user.jid
const ownerNumber = [`${owner}@s.whatsapp.net`, `6281287787370@s.whatsapp.net`, `6281287787370@s.whatsapp.net`]
...
const YtRahmanbotz = fs.readFileSync ('./media/pee.jpg')
...
const katalog = (teks) => {
res = kuza.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "MarmutBotz😈", "thumbnail": fs.readFileSync('./media/pee.jpg'), "surface": 'CATALOG' }}, {quoted:fstatus})
kuza.relayWAMessage(res)}
const fakeyt = (teks) => { 
kuza.sendMessage(from, teks, text,{contextInfo :{text: 'hi', "forwardingScore": 1000000000, isForwarded: false, sendEphemeral: false, "externalAdReply": { "title": `hallo ${pushname}🗿` , "body": `YT : rahman bacot`, "mediaType": "2", "thumbnailUrl": "https://c.top4top.io/p_2087f30hj1.jpeg", "mediaUrl": "https://tps.com/channel/UCJPqI5eVhKPXPL2V8y6pIDA", "thumbnail": fs.readFileSync('./media/pee.jpg'), "sourceUrl": "", },mentionedJid:[sender]}, quoted : fstatus})};
...
const fvn = {key: {participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6281287787370-1613049930@g.us" } : {})},message: { "audioMessage": {"mimetype":"audio/ogg; codecs=opus","seconds":99999,"ptt": "true"}} }
const fstatus = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "YT : rahman bacot", 'jpegThumbnail': fs.readFileSync('./media/pee.jpg')}}}
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            thumbnail: YtRahmanbotz, //Gambarnye
...
run = process.uptime();
 ...
 footerText: '🌀 𝘊𝘳𝘦𝘢𝘵𝘦𝘥 𝘉𝘺 Rahman𝘉𝘰𝘵𝘻',
...
case 'bc':
			...
			"footerText": 'MarmutBot by Rahmanbotz',
			"buttons": [
...
costum(buffer, sticker, Verived, `Yt rahman bacot`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
case 'antilink':
  ...
  case 'p': case 'proses':
    ...
    if (getTextP !== undefined) {
        mentions(getTextP.replace('pesan', quotedMsg.chats).replace('nama', quotedMsg.sender.split("@")[0]).replace('jam', jmn).replace('tanggal', date), [quotedMsg.sender], true);
    } else {
...
case 'd': case 'done':
    ...
    if (getTextD !== undefined) {
        mentions(getTextD.replace('pesan', quotedMsg.chats).replace('nama', quotedMsg.sender.split("@")[0]).replace('jam', jmn).replace('tanggal', date), [quotedMsg.sender], true);
    } else {
